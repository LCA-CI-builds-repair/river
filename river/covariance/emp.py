...
def _update_from_state(self, n: int, mean: dict, cov: dict):
...
cov : dict
A dictionary of covariance values.
...
self._cov[i, j] = stats.covariance(mean[i], mean[j], ddof=self.ddof)
if 'cov' not in cov:
cov_ = np.cov(np.array([mean[i], mean[j]]).T, ddof=self.ddof)[0, 1]
else:
cov_ = cov[(i, j)] if (i, j) in cov else cov[(j, i)]
self._cov[i, j] += cov_
...
self._cov[i, i] = stats.variance(mean[i], ddof=self.ddof)
if i in cov:
cov_ = cov[i]
else:
cov_ = 0.0
